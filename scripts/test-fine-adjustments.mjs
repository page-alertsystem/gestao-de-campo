...
try {
  ...
  // APR approved: multiple registered/external technicians, repeated front/back sheets,
  ...
  assert.equal(await page.locator('.apr-sheet-card').count(), 1)
  await page.getByLabel('Frente da folha 1').setInputFiles('public/alert-logo.png')
  await page.getByRole('img', { name: 'Frente da folha 1' }).waitFor()
  await page.getByLabel('Verso da folha 1').setInputFiles('public/alert-logo.png')
  await page.getByRole('img', { name: 'Verso da folha 1' }).waitFor()
  await page.getByRole('button', { name: 'Adicionar outra folha' }).click()
  assert.equal(await page.locator('.apr-sheet-card').count(), 2)
  await page.getByLabel('Frente da folha 2').setInputFiles('public/alert-logo.png')
  await page.getByRole('button', { name: 'Concluir APR' }).click()
  await page.getByText('Adicione a frente e o verso da folha 2.').waitFor()
  await page.getByLabel('Verso da folha 2').setInputFiles('public/alert-logo.png')
  await page.getByRole('img', { name: 'Verso da folha 2' }).waitFor()
  await page.setViewportSize({ width: 390, height: 844 })
  await page.screenshot({ path: path.join(output, 'apr-form-mobile.png'), fullPage: true, animations: 'disabled' })
  await page.locator('.apr-sheet-list').screenshot({ path: path.join(output, 'apr-sheets-mobile.png'), animations: 'disabled' })
  ...
  assert.equal(aprState.aprRecords[0].backPhotoCount, 2)
