...
try {
  ...
  const signAndSaveLadder = async name => {
    await page.locator('.signature-select').click()
  ...
  const finishLadder = async name => {
    ...
    if (await page.getByRole('dialog', { name: 'Revisão final das escadas' }).isVisible()) await page.getByRole('button', { name: 'Continuar para assinatura' }).click()
    await signAndSaveLadder(name)
  }
  await enterLadder()
  await page.locator('.ladder-question').nth(5).getByRole('button', { name: 'Não conforme', exact: true }).click()
  assert.equal(await page.getByRole('alertdialog').count(), 0)
  assert.equal((await state()).inventory.find(item => item.category === 'Escada').ladderRestriction, undefined)
  ...
  assert.equal((await state()).inventory.find(item => item.category === 'Escada').ladderRestriction, undefined)
  // An accidental selection, even followed by cancelling, must have no impact.
  await page.getByRole('button', { name: 'Iniciar auditoria' }).click()
  await page.locator('.ladder-question').nth(5).getByRole('button', { name: 'Não conforme', exact: true }).click()
  ...
  // Review every failure only after all answers; corrections must update the list.
  await enterLadder()
  for (const [i, question] of (await page.locator('.ladder-question').all()).entries()) {
    await question.getByRole('button', { name: i === 1 || i === 5 ? 'Não conforme' : i >= 8 ? 'Não aplicável' : 'Conforme', exact: true }).click()
  }
  await page.getByLabel('Observação do equipamento (opcional)').fill('Escada de teste: substituição solicitada ao gestor.')
  await page.locator('input[type=file]').setInputFiles('public/alert-logo.png')
  assert.equal(await page.getByRole('dialog').count(), 0)
  await page.getByRole('button', { name: 'Concluir checklist' }).click()
  const review = page.getByRole('dialog', { name: 'Revisão final das escadas' })
  await review.waitFor()
  assert.equal(await review.locator('li').count(), 2)
  assert.ok((await review.innerText()).includes('Foram identificadas as seguintes não conformidades'))
  assert.equal((await state()).inventory.find(item => item.category === 'Escada').ladderRestriction, undefined)
  await review.getByRole('button', { name: 'Revisar esta escada' }).click()
  assert.equal(await page.getByLabel('Observação do equipamento (opcional)').inputValue(), 'Escada de teste: substituição solicitada ao gestor.')
  await page.locator('.ladder-question').nth(1).getByRole('button', { name: 'Conforme', exact: true }).click()
  await page.getByRole('button', { name: 'Concluir checklist' }).click()
  await review.waitFor()
  assert.equal(await review.locator('li').count(), 1)
  assert.ok(!(await review.locator('li').innerText()).includes('TAG'))
  await page.setViewportSize({ width: 390, height: 844 })
  await page.screenshot({ path: path.join(output, 'ladder-final-review-mobile.png'), animations: 'disabled', fullPage: true })
  await page.setViewportSize({ width: 1440, height: 1050 })
  await review.getByRole('button', { name: 'Continuar para assinatura' }).click()
  assert.equal((await state()).inventory.find(item => item.category === 'Escada').ladderRestriction, undefined)
  await signAndSaveLadder('ladder-blocked.pdf')
  ...
  const finalRestriction = (await state()).inventory.find(item => item.category === 'Escada').ladderRestriction
  assert.equal(finalRestriction.status, 'Não liberada')
  assert.equal(finalRestriction.questions.length, 1)
  assert.equal(finalRestriction.reportedAt, ladderAudit.completedAt)
  ...
  // Review an earlier ladder without losing the later ladder's answers/photo.
  const multiSeed = structuredClone(data)
  multiSeed.people[0].groups = ['Técnico']
  const secondLadder = { ...inventory[2], id: 'qa-ladder-second', code: 'QA-SECOND', equipment: 'Segunda escada' }
  multiSeed.inventory.push(secondLadder)
  multiSeed.stockAssignments.push({ ...assignments[2], id: 'qa-assignment-second', inventoryItemId: secondLadder.id, code: secondLadder.code, equipment: secondLadder.equipment })
  await page.evaluate(async seed => {
    await new Promise((resolve, reject) => { const open = indexedDB.open('gio-local-v1', 1); open.onsuccess = () => { const db = open.result; const tx = db.transaction('application', 'readwrite'); tx.objectStore('application').put(seed, 'state'); tx.oncomplete = () => { db.close(); resolve() }; tx.onerror = reject } })
  }, multiSeed)
  await page.reload()
  await page.getByRole('heading', { name: 'Bom dia, Pessoa' }).waitFor()
  await enterLadder()
  for (const [i, question] of (await page.locator('.ladder-question').all()).entries()) await question.getByRole('button', { name: i === 1 ? 'Não conforme' : 'Conforme', exact: true }).click()
  await page.locator('input[type=file]').setInputFiles('public/alert-logo.png')
  await page.getByRole('button', { name: 'Próximo equipamento' }).click()
  assert.equal(await page.getByRole('dialog').count(), 0)
  for (const [i, question] of (await page.locator('.ladder-question').all()).entries()) await question.getByRole('button', { name: i === 5 ? 'Não conforme' : 'Não aplicável', exact: true }).click()
  await page.locator('input[type=file]').setInputFiles('public/alert-logo.png')
  await page.getByRole('button', { name: 'Concluir checklist' }).click()
  assert.equal(await review.locator('.ladder-review-item').count(), 2)
  await review.locator('.ladder-review-item').first().getByRole('button', { name: 'Revisar esta escada' }).click()
  await page.locator('.ladder-question').nth(1).getByRole('button', { name: 'Conforme', exact: true }).click()
  await page.getByRole('button', { name: 'Próximo equipamento' }).click()
  assert.equal(await page.locator('.ladder-question').nth(5).getByRole('button', { name: 'Não conforme', exact: true }).getAttribute('aria-pressed'), 'true')
  assert.equal(await page.locator('.audit-photo-preview').count(), 1)
  await page.getByRole('button', { name: 'Concluir checklist' }).click()
  assert.equal(await review.locator('.ladder-review-item').count(), 1)
  assert.ok((await review.locator('.ladder-review-item').innerText()).includes('Segunda escada'))
  await review.getByRole('button', { name: 'Cancelar auditoria', exact: true }).click()
  const cancelledReview = await state()
