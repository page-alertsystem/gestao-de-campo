    ...
    <form className="request-form" onSubmit={submit}>
      ...
      <main className="request-content">
        ...
        <section className="form-section">
          ...
          <div className="request-items">{rows.map((row, index) => <article className="request-item" key={row.id}><div className="request-item-heading"><b>Item {index + 1}</b><button type="button" onClick={() => removeRow(row.id)}><Trash2 size={15} /> Remover</button></div><div className="request-row-grid"><label>Equipamento<input value={row.equipment} onChange={event => updateRow(row.id, 'equipment', event.target.value)} placeholder="O que você precisa?" required /></label><label>Marca (opcional)<input value={row.brand} onChange={event => updateRow(row.id, 'brand', event.target.value)} /></label><label>Modelo (opcional)<input value={row.model} onChange={event => updateRow(row.id, 'model', event.target.value)} /></label><label>Quantidade<input type="text" inputMode="decimal" pattern="[0-9]+([,.][0-9]+)?" title="Digite um número inteiro ou decimal, como 2 ou 1,5" value={row.quantity} onChange={event => updateRow(row.id, 'quantity', event.target.value)} placeholder="Ex.: 2 ou 1,5" required /></label></div></article>)}</div>
