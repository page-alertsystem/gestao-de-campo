...
export function StockManagement({ data, onChange }: { data: AppData; onChange: (data: AppData, message?: string) => void }) {
  ...
  const balances = useMemo(() => {
    ...
    data.stockAssignments.filter(item => item.personId === personId).forEach(item => totals.set(item.inventoryItemId, (totals.get(item.inventoryItemId) ?? 0) + item.quantity))
  ...
  }, [data.inventory, data.stockAssignments, personId])
  ...
  const submit = (event: FormEvent) => {
    ...
    const assignment: StockAssignment = {
      ...
      quantity: amount, assignedAt: new Date().toISOString(), assignedBy: data.account.name, notes: notes.trim(),
    }
    onChange({
      ...data,
    ...
    }, `${selectedItem.equipment} atribuído a ${selectedPerson.name}.`)
    setQuantity('1')
    setNotes('')
  }

  const userCountWithStock = new Set(data.stockAssignments.map(item => item.personId)).size
  return <>
    ...
    <section className="stock-management-grid">
      ...
      <section className="surface user-stock-card">
        ...
        <div className="responsive-table"><table><thead><tr><th>Equipamento</th><th>Categoria</th><th>Quantidade</th></tr></thead><tbody>{balances.length ? balances.map(({ item, total }) => <tr key={item!.id}><td>{item!.equipment}</td><td>{item!.category}</td><td>{total} {item!.unit.toLowerCase()}</td></tr>) : <tr><td colSpan={3} className="table-empty">Este usuário ainda não possui itens atribuídos.</td></tr>}</tbody></table></div>
      </section>
    </section>

    <section className="surface table-surface assignment-history"><div className="table-toolbar"><div><p className="eyebrow">Rastreabilidade</p><h3>Últimas atribuições</h3></div></div><div className="responsive-table"><table><thead><tr><th>Data</th><th>Usuário</th><th>Equipamento</th><th>Quantidade</th><th>Atribuído por</th></tr></thead><tbody>{data.stockAssignments.length ? [...data.stockAssignments].reverse().map(entry => { const person = data.people.find(item => item.id === entry.personId); const item = data.inventory.find(inventory => inventory.id === entry.inventoryItemId); return <tr key={entry.id}><td>{new Date(entry.assignedAt).toLocaleString('pt-BR')}</td><td>{person?.name ?? 'Usuário removido'}</td><td>{item?.equipment ?? 'Item removido'}</td><td>{entry.quantity} {item?.unit.toLowerCase() ?? ''}</td><td>{entry.assignedBy}</td></tr> }) : <tr><td colSpan={5} className="table-empty">Nenhuma atribuição realizada.</td></tr>}</tbody></table></div></section>
  </>
}
